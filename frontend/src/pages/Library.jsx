    ...
    <Layout>
      <div className="mx-auto w-full max-w-6xl pb-12">
        <div className="mb-8 flex items-center gap-4">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-white/10 bg-white/10 text-sky-200">
            <BookOpen size={24} />
          </div>
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white">My Library</h1>
            <p className="font-medium text-slate-300">Browse saved study sessions by topic, reopen earlier analyses, and continue notes from where you left off.</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <CardShell>
            <div className="text-center">
              ...
              <h2 className="mt-4 text-xl font-extrabold text-white">Your library is empty</h2>
          ...
          <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {groupedItems.map((item) => (
              <div key={item.title} className="rounded-[2rem] border border-white/10 bg-white/8 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.18)] backdrop-blur-2xl">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-sky-200">
                  <Layers3 size={20} />
                </div>
                <h2 className="text-xl font-extrabold capitalize text-white">{item.title}</h2>
...
function CardShell({ children }) {
  return <div className="rounded-[2rem] border border-white/10 bg-white/8 p-10 shadow-[0_24px_80px_rgba(0,0,0,0.18)] backdrop-blur-2xl">{children}</div>;
}
